...
impl<T: Trace<T>> Heap<T> {
    ...
    /// Adds a new object to this heap that will be cleared upon the next gc cycle,
    /// if not attached to the object tree
    pub fn insert_temp(&mut self, object: T) -> Handle<T> {
        let ptr = Box::into_raw(Box::new(object));

        let gen = self.new_generation();
        let handle = Handle::new(gen, ptr);
        self.objects.insert(handle);
        
        handle
    }

    /// Adds a new object to this heap that will not be cleared by the gc until all,
    ...
    pub fn insert(&mut self, object: T) -> Rooted<T> {
        ...
        Rooted::new(rc, handle)
    } 

    /// Upgrade a handle (that will be cleared by the gc) into a rotted handle (that will not)
    ...
    /// Get a mutable reference to a heao object
...
impl<T> Handle<T> {
    pub fn new(gen: Generation, ptr: *mut T) -> Self {
        Self { gen, ptr }
    }
...
impl<T> Rooted<T> {
    pub fn new(rc: Rc<()>, handle: Handle<T>) -> Self {
        Self { rc, handle }
    }
